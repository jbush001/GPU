...
class TileBufferReference(implicit cfg: GpuConfig) {
  ...
  val depths = Array.fill(cfg.totalTilePixels)(0xffffff)
...
class TileBufferTests extends AnyFunSuite with ChiselSim with ColorTestHelpers {
  ...
  def writeQuad(dut: TileBuffer, x: Int, y: Int, mask: Int, colors: Seq[ColorRef],
    ...
    for (pixel <- 0 until 4) {
      ...
      dut.io.shadedQuad.bits.depths(pixel).poke(depths(pixel))
    }
  ...
  def flush(dut: TileBuffer, select: RenderBufferId.Type): Seq[Int] = {
    ...
    while (results.length < cfg.totalTilePixels) {
      // Add random delays
  ...
  test("TileBuffer depth check disable") {
    simulate(new TileBuffer()) { dut =>
      ...
      assert(depth(0) == 3)
      assert(depth(1) == 3)
      assert(depth(cfg.tileSizePixels) == 3)
      assert(depth(cfg.tileSizePixels + 1) == 3)
  ...
  test("TileBuffer depth write disable") {
    simulate(new TileBuffer()) { dut =>
      ...
      assert(depth(cfg.tileSizePixels) == 0xffffff)
      assert(depth(cfg.tileSizePixels + 1) == 0xffffff)
